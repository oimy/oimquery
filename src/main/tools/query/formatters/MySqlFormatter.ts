...
export default class MySqlFormatter implements Formatter {
    format(table: Table): string {
        const startSentence =
            DDL_CREATE_TABLE + table.schema + "." + table.name + " (\n";
        ...
        table.columns.forEach((column) => {
            const line = `${INDENT}${column.name} ${column.dataType} ${
                ...
                table.columnKeyOption.primaryKeyColumnName === column.name
                    ? " AUTO_INCREMENT"
                    : ""
            }`;
        ...
        contentLines.push(
            `${INDENT}PRIMARY KEY (${table.columnKeyOption.primaryKeyColumnName})`
        );
        Object.entries(table.columnKeyOption.uniqueKeyIndexAndColumnNamesMap)
            .sort((a, b) => a[0].localeCompare(b[0]))
            .forEach(([index, columnNames]) => {
                contentLines.push(
                    `${INDENT}UNIQUE KEY uk${index} (${columnNames.join(", ")})`
                );
            });
        Object.entries(table.columnKeyOption.indexKeyIndexAndColumnNameMap)
            ...
            .forEach(([index, columnNames]) => {
                contentLines.push(
                    `${INDENT}KEY idx${index} (${columnNames.join(", ")})`
                );
            });
