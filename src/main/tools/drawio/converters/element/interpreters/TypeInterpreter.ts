import { BLANK, NULLABLE } from "../../../../constants";
...
const NUMBER_DATA_TYPES: Readonly<string[]> = [
    "bigint",
    "integer",
    "smallint",
    "double",
...
export class TypeInterpreter implements Interpreter<TypeInterpretResult> {
    ...
    interpret(rowElement: RowElement): TypeInterpretResult {
        if (!this.checkDataType(rowElement.type)) {
            return TypeInterpretResult.ofFail([
                new NotSupportedTypeFailure(rowElement.type),
            ]);
        }
